...
class RecogEigenFaces:
    ...
    def load_trained_data(self):
        ...
        for (subdirs, dirs, files) in os.walk(self.face_dir):
            for subdir in dirs:
                ...
                key += 1
        self.names = names 
        self.model.load('eigen_trained_data.yml')
    ...
    def process_image(self, inImg):
        ...
        for i in range(len(faces)):
            face_i = faces[i]
            x = max((face_i[0] * RESIZE_FACTOR), 0)
            y = max((face_i[1] * RESIZE_FACTOR) - 10, 0)
            w = min((face_i[2] * RESIZE_FACTOR), gray.shape[1])
            h = min((face_i[3] * RESIZE_FACTOR) + 25, gray.shape[0])
